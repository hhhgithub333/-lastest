"""
benchmark_qwen.py
Qwen 专项评测脚本（通过后端 API，含情感识别）

用法：
  python benchmark_qwen.py                       # 完整流程（中文）
  python benchmark_qwen.py --lang english        # 完整流程（英文）
  python benchmark_qwen.py --tts-only            # 仅生成音频
  python benchmark_qwen.py --stt-only            # 仅 STT 评估
"""
...
def run_tts(texts: list, engine: str = "qwen", voice: str = DEFAULT_VOICE, lang_label: str = "中文") -> list:
    results = []
...
def main():
    parser = argparse.ArgumentParser(description="Qwen 专项评测（含情感识别）")
    ...
    parser.add_argument("--engine", default="qwen", help="TTS 引擎名称")
