"""
benchmark_xtts_v2.py
XTTS-v2 专项评测脚本（通过后端 API，含情感识别）

用法：
  python benchmark_xtts_v2.py                       # 完整流程（中文）
  python benchmark_xtts_v2.py --lang english        # 完整流程（英文）
  python benchmark_xtts_v2.py --tts-only            # 仅生成音频
  python benchmark_xtts_v2.py --stt-only            # 仅 STT 评估
"""
...
def run_tts(texts: list, engine: str = "xtts_v2", lang_label: str = "中文") -> list:
    """批量调用 TTS API 生成音频"""
    results = []
...
def build_report(results: list) -> dict:
    ...
    error_samples = [r for r in successful if not r.get("is_match")]
    ...
    report = {
        ...
        "error_rate": round(len(error_samples) / len(successful), 4) if successful else None,
        "avg_rtf": mean(rtfs),
...
def main():
    parser = argparse.ArgumentParser(description="XTTS-v2 专项评测（含情感识别）")
    ...
    parser.add_argument("--engine", default="xtts_v2", help="TTS 引擎名称")
