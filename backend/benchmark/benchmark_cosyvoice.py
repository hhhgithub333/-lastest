"""
benchmark_cosyvoice.py
CosyVoice 专项评测脚本（通过后端 API，含情感识别）

前置条件：
  后端服务已启动：uvicorn app.main:app --reload

用法：
  python benchmark_cosyvoice.py                       # 完整流程（中文）
  python benchmark_cosyvoice.py --lang english        # 完整流程（英文）
  python benchmark_cosyvoice.py --tts-only            # 仅生成音频
  python benchmark_cosyvoice.py --stt-only            # 仅 STT 评估
"""
...
def run_tts(texts: list, engine: str = "cosyvoice", voice: str = DEFAULT_VOICE, lang_label: str = "中文") -> list:
    results = []
...
def main():
    parser = argparse.ArgumentParser(description="CosyVoice 专项评测（含情感识别）")
    ...
    parser.add_argument("--engine", default="cosyvoice", help="TTS 引擎名称")
    ...
    if args.stt_only:
        ...
        tts_results = run_tts(texts, engine=args.engine, voice=args.voice, lang_label=lang_label)
        if args.tts_only:
            return

    stt_results = run_stt(tts_results, language=stt_lang)
