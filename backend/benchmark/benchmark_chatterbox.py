"""
python benchmark\benchmark_chatterbox.py --lang english
ChatterBox 专项评测脚本 - 通过后端 API 调用（含情感识别）
"""
...
def run_tts(texts: list, engine: str = "chatterbox", lang_label: str = "中文") -> list:
    """批量调用 TTS API 生成音频"""
    results = []
    ...
    for i, text in enumerate(texts):
        ...
        result = call_tts_api(
            text,
            engine=engine,
            ref_audio_path=None,  # 不传固定参考音频，让引擎根据 emotion 自动选择
            emotion=emotion,
...
def main():
    parser = argparse.ArgumentParser(description="ChatterBox 专项评测（含情感识别）")
    ...
    parser.add_argument("--engine", default="chatterbox", help="TTS 引擎名称")
